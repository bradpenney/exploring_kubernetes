...
**CRITICAL: Never suggest running persistent databases on Kubernetes**

- ❌ **DO NOT** use persistent databases as examples (MongoDB, PostgreSQL, MySQL, MariaDB, etc.)
- ❌ **DO NOT** suggest deploying primary data stores to Kubernetes
- ✅ **DO** use appropriate examples: web servers (nginx), caching layers (Redis), monitoring tools (Prometheus, Grafana), ingress controllers, message queues

**Why this matters:** Most large enterprises recognize that running persistent databases on Kubernetes is an anti-pattern. Persistent databases require specialized infrastructure, backup strategies, and operational expertise that Kubernetes adds complexity to rather than simplifying.

**Acceptable examples for Helm/kubectl tutorials:**
- **Web servers:** nginx, apache, httpd
- **Caching/session stores:** Redis, Memcached (these run well on K8s!)
- **Monitoring:** Prometheus, Grafana, metrics-server
- **Ingress:** nginx-ingress, traefik
- **Message queues:** RabbitMQ, Kafka (infrastructure components)
- **Proxies and API gateways:** Envoy, Kong

**NOT acceptable:**
- PostgreSQL, MySQL, MariaDB, MongoDB, CouchDB, Cassandra (persistent databases)
